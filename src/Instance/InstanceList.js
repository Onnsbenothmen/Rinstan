...
const InstanceList = () => {
    ...
    useEffect(() => {
        ...
        const fetchInstances = async () => {
            try {
                const response = await axios.get('http://localhost:3000/instances');
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    axios.delete(`http://localhost:3000/instances/${id}`);
    ...
    const handleFormSubmit = async (values) => {
        try {
            await axios.put(`http://localhost:3000/instances/${instanceToEdit.id}`, values);
